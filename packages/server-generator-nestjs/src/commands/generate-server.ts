...
async function copyAllFiles(serverDir: string): Promise<void> {
  ...
  debugFiles('Copying main.ts...');
  await copyRootFilesFromSrc(path.join(packageDistRoot, 'src', 'main.ts'), path.join(serverDir, 'src'));

  debugFiles('Copying mikro-orm.config.ts...');
  await copyRootFilesFromSrc(path.join(packageDistRoot, 'src', 'mikro-orm.config.ts'), path.join(serverDir));

  debugFiles('Copying domain shared files...');
  await copySharedAndRootFiles(
    path.join(packageDistRoot, 'src', 'shared'),
    path.join(serverDir, 'src', 'domain', 'shared'),
  );
}

async function writeConfigurationFiles(serverDir: string, _absDest: string): Promise<void> {
  ...
  debugFiles(`Writing .gitignore... to ${serverDir}`);
  await writeGitignore(serverDir);
}
...
async function writePackage(dest: string): Promise<void> {
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      test: 'vitest run',
      dev: 'ts-node --transpile-only --watch src/main.ts',
    ...
    devDependencies: resolveDeps(['@mikro-orm/cli', '@nestjs/cli', '@nestjs/testing', 'typescript', 'vitest', 'tsx']),
  } as const;
...
async function writeTsconfig(dest: string): Promise<void> {
  const tsconfig = {
    ...
    include: ['src/**/*', 'vitest.config.ts', 'mikro-orm.config.ts'],
...
`;
  ...
  await writeFile(path.join(dest, '.gitignore'), gitignoreContent, 'utf-8');
}
