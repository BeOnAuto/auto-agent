...
1. Call `auto_get_model_overview` for a compact view of the full scope (~10K chars). This returns requirements, narratives, scenes, moments, actors, entities, messages, and app shell description — everything you need to understand WHAT to build.
2. Call `auto_get_design` to get theme tokens — generate `theme.css` with CSS custom properties from `theme.colors`, `theme.radius`, `theme.shadow`, and `theme.font` immediately. The model's theme is authoritative.
3. Call `auto_get_changes` to check for pending deltas.
...
5. For each scene to build, call `auto_get_scene_detail(sceneName)` to get the full UI specs, server specs, and message contracts for that specific scene. Build scene by scene to keep context focused.
6. Follow the auto-build skill for all code generation:
...
7. Run all tests for the affected scope.
8. **Browser verification** — after completing each scene, verify the running application:
...
9. If any model inconsistencies are found, correct them and validate via `auto_send_model`.
10. If dev servers are running, report their URLs via `auto_update_endpoints`. If the call fails due to a disconnected WebSocket, note which endpoints need to be reported — once the user reconnects (via `/auto-agent:connect`), immediately re-call `auto_update_endpoints` with those endpoints.
11. Show the **loopback URLs** returned by `auto_update_endpoints` to the user — these are the URLs that open the running app inside Auto (e.g. `https://app.on.auto/{workspaceId}/agent/{sessionId}/{label}`). Always show these instead of raw localhost URLs. The tool response contains the loopback URLs — parse and display them verbatim. For frontend apps, show the web application URL. For GraphQL APIs, show the playground. For REST APIs, show Swagger or the base API URL. If the initial call failed and you re-called after reconnection, show the loopback URLs from the successful call at that point.
12. Summarize: slices generated/updated, tests passing/failing, browser verification results, any model corrections made.
