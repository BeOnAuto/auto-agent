...
function createHookState(ketchupDir) {
  if (!fs2.existsSync(ketchupDir)) {
    fs2.mkdirSync(ketchupDir, { recursive: true });
  }
  const stateFile = path2.join(ketchupDir, ".claude.hooks.json");
  function read() {
...
function handleStop(ketchupDir, input2) {
...
async function resolvePaths(claudeDir2) {
  ...
  const ketchupDir = path3.join(projectRoot, ketchupDirName);
  return {
    projectRoot,
    claudeDir: claudeDir2,
    ketchupDir,
    remindersDir: path3.join(ketchupDir, "reminders"),
    validatorsDir: path3.join(ketchupDir, "validators")
  };
}
