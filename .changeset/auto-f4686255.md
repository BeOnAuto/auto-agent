...
- Added job graph processor package for orchestrating dependent job workflows with DAG validation, failure policies (halt, skip-dependents, continue), retry with exponential backoff, and per-job timeouts
- Added correlation-based event routing to message bus with exact match and prefix subscription methods
- Extended pipeline to pass shared infrastructure (event store, message bus) to command handlers via context
